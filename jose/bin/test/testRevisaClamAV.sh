...
for home in $HOMES
do
    FICH_LOG="$(find $home/.clamtk/history/ -type f -printf "%T+ %p\n" | sort -r | head -1 | cut -d' ' -f2)"
    # Ultimo fichero en el directorio de log de clamav
    if [ -z $FICH_LOG -o ! -r $FICH_LOG ] # Cadena vacia o fichero no legible
    then 
        printf "%s\n" "No existe fichero $FICH_LOG o no legible"
        ls -l $FICH_LOG
        continue 
    fi
    printf "%s\n" "Procesar $FICH_LOG ..."
...
INFECTADOS=$(grep FOUND $LISTA_LOGS | cut -d: -f 1)
